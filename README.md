...
**`run` without `--track`:** when `track_id` is unset/empty, **or** the run is Idle after `workflow: backlog clear` (no `failure_class`, no `next_track`), start the first exact **`Ready — not started`** row in that project's `conductor/conductor.md` (never Proposed / HITL / trailing notes; fail closed → pass `--track`). Every other Idle/Stopped case with a set `track_id` **retains** (Stopped restart, Idle after failure). Explicit `--track` always wins (including Proposed / probes). Fresh `run` **clears** `next_track` (stale Planner handoff). Omit on Orca today starts `0099` if it remains the only exact Ready; `--track 0099` / `0899` stay the safe explicit probe path.

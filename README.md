...
Phase start / end / stall / stop append a timestamped line to `{workspace}/status.md` (Helping Hands: `C:\dev\Helping-Hands\status.md`). That file stays outside `hands\`.
...
**Four clocks (do not collapse):** (1) CLI poll budget — `wait --timeout-secs` (default **3600**) or optional `run --timeout-secs N` (`N>0`). Bare `run` has **no** poll budget and ticks until Idle/Stopped. Budget expiry is exit **2**, run unchanged, **no abort**. (2) phase wall clock — project `phase_timeouts_secs` (then machine, then table; env still wins) — `failure_class=timeout` + Stopped + `FAILURE.md` **and** abort/recycle the in-flight Prompt; (3) ACP `session/prompt` timeout — Prompt error mapped onto the phase **and** recycle so the next `start` is a new `session/new`; (4) **progress stall** (default **600s**) — adapter `session/update` / inject heartbeat went silent. First stall this phase recycles unless sidecar **`tool_in_flight`**: cancel then recycle (`last_event` = `recycle: stall — new session`), stay **Running**, no `FAILURE.md`. `session/update` text/chunks are **not** a skip. A live ACP tool or TerminalHub child (`exit` still none / `wait_for_exit` outstanding) sets `tool_in_flight` and the first stall **surfaces only** (`watchdog: stall`). Second stall: surface only until the phase wall clock. Override via `COORDINATOR_PROGRESS_STALL_SECS` or machine `progress_stall_secs` (`0` disables). One-shot `cross-model-review` / `plan-review` children use the **same 600s knob** against **stdio/artifact** progress; a stall **kills that child** (tree) and 0011 falls through / plan-review degrades. It is **not** ACP `recycle:` / `tool_in_flight`. `0` disables both. Cancel wait: `COORDINATOR_CANCEL_WAIT_SECS` (default **10**; `0` recycles immediately). Operator `stop` still leaves sessions for attach. Ctrl-C during a ticking `run`/`wait` also leaves the run **Running** (no abort, no artifact).

**`run` without `--track`:** when `track_id` is unset/empty, **or** the run is Idle after `workflow: backlog clear` (no `failure_class`, no `next_track`), start the first exact **`Ready — not started`** row in that project's `conductor/conductor.md` (never Proposed / HITL / trailing notes; fail closed → pass `--track`). Adapter omit-pick (CLI / HTTP `POST /v1/run` / UI) also skips a Ready row whose `track(NNNN):` PR is already merged on the default branch (`gh pr list --state merged`; fail-open if `gh` is missing or errors). `finish_advance` skip is local only (`ci.merge` `done`/`queued` on this run, or a **Completed** conductor row) — no `gh` under the apply lock. Every other Idle/Stopped case with a set `track_id` **retains** (Stopped restart, Idle after failure). Explicit `--track` always wins (including Proposed / probes / already-merged ids). Fresh `run` **clears** `next_track` (stale Planner handoff). Omit on Orca today starts `0099` if it remains the only exact Ready; `--track 0099` / `0899` stay the safe explicit probe path.
...
| Target | Persisted `RunState.ci` hint → `gh pr view` → `gh pr list --head` **open**, then **`--state merged --head`**, then 0046 `track(NNNN):` merged title probe. GitHub **MERGED** resolves immediately (`ci-wait: merged #{n}`, no squash). Hinted `pr view` miss is not sticky (falls through; does not treat the PR oid as default-branch `HeadSha`). A clean local `track/NNNN` or `track/NNNN-*` branch (the subject may be prose) on a non-default branch with no PR is pushed via the resolved GitHub remote (git push order, not hardcoded `origin`) and `gh pr create --title` `track(NNNN): …`, then the existing poll (`ci-wait: opened #{n}`). A persisted `head_sha` is not a target unless that local track tip is an ancestor of the resolved default-branch sha, or no such local branch exists. Feature branch with neither open nor merged proof, and not auto-publishable, stays pending (`ci-wait: waiting for PR`) until the 3600s phase timeout. Default-branch `HEAD` sha still uses `gh run list` |
| Checks | Fail-set is `gh pr checks {n} --required --json bucket,name,state`. `mergeStateStatus` from `gh pr view` is the empty-set backstop (`UNKNOWN`/`BLOCKED` wait; `CLEAN`/`UNSTABLE`/`HAS_HOOKS` fall back to all-checks). `Required` + empty + `Unspecified` waits (same tick after `gh pr create`). Squash `base branch policy prohibits the merge` / missing required checks retries as Pending (not `ci_failed`). Repos with no required contexts keep the 0010 all-checks view. Duplicate rows of the same check **name** on one SHA collapse first (any `pass` wins; `pending`/`cancel` wait; fail only when every twin is completed non-green). A suppressed twin is recorded on `last_summary` as `disagreed:`. Non-required `fail`/`cancel` do **not** `ci_failed`. Draft PR stays pending (`ci-wait: waiting (draft PR)`). Already `MERGED` is green. Required `fail` → `ci_failed`. Required `cancel` → Pending (`ci-wait: waiting (cancelled: …)`), no merge past a cancelled required gate, bounded by the 3600s phase clock. `CheckView::Unspecified` empty scripted list is green |
...
**Apply (single path):** leftover `stub:*` success → Idle / `stub:completed` (Paused stays Paused). Canonical success → **successor phase, stay Running** (Paused stays Paused). Canonical failure → Stopped and **keeps the failed phase id**, except GateFail on `cross-model-review` with `address_findings_attempts < 2` which stays Running/Paused at `address-findings` (see Canonical workflow). Operator `stop` still sets `stub:stopped`. Idle/Stopped and phase mismatch reject for CLI/HTTP. After apply: history best-effort → `current.applied.json` → remove `current.json` → hash on run-state.

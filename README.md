...
| Checks | Fail-set is `gh pr checks {n} --required --json bucket,name,state`. `mergeStateStatus` from `gh pr view` is the empty-set backstop (`UNKNOWN`/`BLOCKED` wait; `CLEAN`/`UNSTABLE`/`HAS_HOOKS` fall back to all-checks). Repos with no required contexts keep the 0010 all-checks view. Non-required `fail`/`cancel` do **not** `ci_failed`. Draft PR stays pending (`ci-wait: waiting (draft PR)`). Already `MERGED` is green. Required `fail`/`cancel` → `ci_failed`. Unspecified/empty scripted list is green |

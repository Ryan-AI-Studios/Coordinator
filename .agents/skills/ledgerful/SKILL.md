...
description: >
  ...
  For Coordinator: run all ledgerful CLI with cwd C:\dev\coordinator\coordinator (init there;
  never in planning root C:\dev\coordinator). Before edits: doctor --json then change-context --json.
---
...
| | |
|---|---|
| **Init** | `ledgerful init` only in **`C:\dev\coordinator\coordinator`** |

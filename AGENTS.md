...
Control Plane entrypoints: `project add|list|show|set|scan`, `run|status|pause|resume|stop` (`run --driver adapter|file_wait|stub`), `outcome write|show`, `failure show`, `wait`, `harness grok start|prompt|compact|status|shutdown`, `serve` (127.0.0.1:7420). Layout/profile tests: `cargo test layout`, `cargo test scan`, `cargo test registry`. Phase Outcome tests: `cargo test outcome`. Harness tests: `cargo test harness`. Workflow tests: `cargo test workflow`. Notify tests: `cargo test notify`. Env overrides: `COORDINATOR_HOME`, `COORDINATOR_STATE_DIR`, `COORDINATOR_STUB_PHASE_TIMEOUT_SECS`, `COORDINATOR_PHASE_TIMEOUT_SECS`, `COORDINATOR_WORKFLOW_DRIVER`, `COORDINATOR_OUTCOME_POLL_MS`, `COORDINATOR_NOTIFY` (`off` disables Windows toasts), `COORDINATOR_GROK_BIN`, `COORDINATOR_GROK_LIVE`. Live Grok smoke (ignored by default): `$env:COORDINATOR_GROK_LIVE='1'; cargo test grok_live -- --ignored --nocapture`. See product README for Layout Profiles, Phase Outcome, the Grok ACP adapter, the canonical workflow, and Failure Artifact / toast.
